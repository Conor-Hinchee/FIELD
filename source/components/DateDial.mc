...
// DateDial — a beveled sub-dial matching the battery/weather bezels, seated
// between the 9 and 10 o'clock indices. Shows weekday + day of month.
class DateDial {

    const POS_FRAC = 9.5 / 12.0;  // clock position (between 9 and 10)
    const POS_R    = 110;         // sub-dial center distance from dial center
    const DIAL_R   = 54;          // sub-dial radius (matches battery/weather)
    ...
    function draw(dc, cx, cy) {
        ...
        // black face with a plain 3px gray outline (no bevel)
        ...
        dc.drawText(dcx, dcy - 15, vfont(dc, (dc.getFontHeight(Graphics.FONT_XTINY) * 11) / 20),
                    dowName(now.day_of_week),
                    Graphics.TEXT_JUSTIFY_CENTER | Graphics.TEXT_JUSTIFY_VCENTER);

        // day of month (large, below)
        dc.setColor(Palette.PRIMARY, Graphics.COLOR_TRANSPARENT);
        dc.drawText(dcx, dcy + 9, vfont(dc, (dc.getFontHeight(Graphics.FONT_XTINY) * 7) / 6),
                    now.day.toString(),
                    Graphics.TEXT_JUSTIFY_CENTER | Graphics.TEXT_JUSTIFY_VCENTER);
    }
